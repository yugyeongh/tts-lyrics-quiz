...
if st.session_state.tts_state == 'loading':
    ...
    icon = st.session_state.icon_override or "🔊"
    if "정답입니다" in st.session_state.answer_result:
        icon = "✅"
        st.session_state.icon_override = None
    elif "오답입니다" in st.session_state.answer_result:
        icon = "❌"
        st.session_state.icon_override = None
                ...
                with sr.AudioData(audio_data, 16000, 2) as audio:
                    try:
                        ...
                        if "정답" in text:
                            ...
                            st.session_state.icon_override = "💡"
                            in_answer_mode = True
                        elif in_answer_mode:
